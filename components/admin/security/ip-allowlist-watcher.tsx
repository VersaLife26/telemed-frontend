 ...
 * enough not to hammer the API and fast enough that nobody reaches for the
 * reload button first.
 */
export function IpAllowlistWatcher() {
  ...
  const check = React.useCallback(async () => {
    ...
    try {
      ...
      setState(body.allowed ? "allowed" : "blocked");
    } catch {
      setState("unknown");
    }
  }, []);
    ...
    return (
      <Alert variant="success">
        ...
        <AlertDescription className="space-y-2">
          <p>The API now accepts requests from this address.</p>
      ...
      <p className="text-sm text-muted-foreground">
        {state === "checking"
          ...
          : state === "blocked"
            ...
            : "Could not reach the API to check. It may be down, or unreachable from this network."}
      </p>
